...
/*
  Reason for those functions is my laziness when copying test vectors.
*/
func TEST(s string) []byte {
	return []byte(s)
}

func TEST0(s string) []byte {
	ret := []byte(s)
	ret = append(ret, uint8(0))
	return ret
}

func R10(s string) string {
	ret := ""
	for i := 0;i<10;i++ {
		ret += s
	}
...
//	Taken from: http://isthe.com/chongo/src/fnv/test_fnv.c
//	Thanks to mr. Fowler, Noll and Vo for making that public domain
var fnv_test_str = [][]byte{
  TEST(""),
  TEST("a"),
  TEST("b"),
  TEST("c"),
